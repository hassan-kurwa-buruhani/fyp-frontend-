...
export default function StudentAnswerSheet() {
  ...
  const fetchAnswers = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${baseUrl}/exams/${id}/student-answers/`, {
        headers: {
  ...
  onPress={() => router.push({ 
    pathname: "/(student)/student-answer", 
    params: { examId: id } 
  })}
>
  <MaterialIcons name="assignment" size={20} color="#fff" />
  <Text style={styles.actionButtonText}>View Marking Details</Text>
</TouchableOpacity>
